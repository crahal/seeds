...
main_fun <- function(data, seed) {
  ...
  data_short <- data[, c("fctvho00r", "fccomh00r", "fccmex00r", "fcinth00r",
                         ...
                         "fpwrdscm", "fdacaq00", "fd05s00", "fpclsi00",
                         "fpchti00", "fdkessl", "fdtots00", "foede000", "tech")]
...
private.data.dir <- file.path(getwd(), "..", "data", "mcs")
...
fmt_time <- function(sec) {
  if (sec < 60) return(sprintf("%0.1fs", sec))
  if (sec < 3600) return(sprintf("%0.1fmin", sec / 60))
  return(sprintf("%0.1fh", sec / 3600))
}

total_iters <- length(seeds_to_use)

for (idx in seq_along(seeds_to_use)) {
  seed <- seeds_to_use[[idx]]
  main_fun(data, seed)
  elapsed <- as.numeric(difftime(Sys.time(), start_time, units = "secs"))
  rate <- elapsed / idx
  eta <- (total_iters - idx) * rate
  cat(sprintf("[iter %s/%s | seed %s | elapsed %s | eta %s]\n",
              idx, total_iters, seed, fmt_time(elapsed), fmt_time(eta)))
  flush.console() # force immediate display in interactive sessions
}
